...
class NormalizingFlow(REINFORCE):
    ...
    def load_model(self, scenario_configs=None):
        # AAAI-IRT patch: train_scenario 흐름의 check_continue_training이 인자
        # 없이 호출. scenario_configs=None이면 graceful skip.
        if scenario_configs is None:
            self.continue_episode = 0
            self.logger.log('>> No prior nf checkpoint passed; training from scratch.', 'yellow')
            return

...
plan §4·§13의 셋째 기여(표준적·단순 생성기 묶음 위 SUT-불변 측정)를 회복하기
위한 자리. SafeBench의 `adv_behavior_single` 시나리오 정의(`update_behavior`가
`convert_actions: speed = action[0]*5 + 5`로 background actor target_speed
조절)와 함께 작동한다.

우리 c 다이얼은 action[0] 한 자리 변수로 매핑된다. c=0.0(부드러운 진입)부터
c=4.0(빠른 진입)까지 background actor가 ego 진로에 단조 강하게 진입한다.
Wang·Ma·Lai 2024의 공격성 파라미터와 같은 자리이고, RL 학습이 봉인한
...
class IDMAttackPolicy(BasePolicy):
    ...
    def train(self, replay_buffer):
        # type='unlearnable'이라 학습 자리 없음. 일관성 위해 pass.
        pass
    ...
    def get_action(self, state, infos, deterministic=False):
        ...
        `_aaai_action_value`를 주입한 자리에 따라 c별로 다른 속도가 나온다.
        """
    ...
    def load_model(self, scenario_configs=None):
        # 학습 ckpt 없음. graceful skip(train_scenario 흐름에서 호출되는 자리 +
        # eval 흐름에서 호출되는 자리 둘 다).
        return None

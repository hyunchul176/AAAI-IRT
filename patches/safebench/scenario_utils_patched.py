...
def scenario_parse(config, logger):
    ...
    with open(list_of_scenario_config, 'r') as f:
        ...
        # AAAI-IRT patch: filter by data_id so the orchestrator can map one cell
        # to exactly one (scenario_id, route_id, data_id) rollout. Without this
        # the yaml's (scenario_id, route_id) leaves a 10-data_id group running
        # together, breaking our cell -> SafeBench eval 1-to-1 mapping.
